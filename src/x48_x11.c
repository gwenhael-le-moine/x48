...
button_t buttons_sx[] = {
    ...
    {"ALPHA", 0, 0,           0x35,         0,          250, 36,    26, WHITE,
     0,       0, alpha_width, alpha_height, alpha_bits, 0,   "USR", 0,  "ENTRY",
     0,       0},
...
button_t buttons_gx[] = {
    ...
    {"LEFT", 0, 0, 0x62,       150,         100,       36,  26,
     ...
     0,      0, 0, 0},
    {"DOWN", 0,          0,           0x61,      200, 100,    36, 26, WHITE, 0,
     0,      down_width, down_height, down_bits, "Q", "VIEW", 0,  0,  0,     0},
    {"RIGHT", 0, 0,           0x60,         250,        100, 36,     26, WHITE,
     0,       0, right_width, right_height, right_bits, "R", "SWAP", 0,  0,
     0,       0},
    ...
    {"ENTER", 0, 0, 0x44, 0, 200,        86, 26,       WHITE, "ENTER",
     2,       0, 0, 0,    0, "EQUATION", 0,  "MATRIX", 0,     0},
    {"NEG", 0,         0,          0x43,     100, 200,    36, 26,    WHITE, 0,
     0,     neg_width, neg_height, neg_bits, "Y", "EDIT", 0,  "CMD", 0,     0},
    {"EEX", 0, 0, 0x42, 150, 200,    36, 26,    WHITE, "EEX",
    ...
    {"PLUS", 0,          0,           0x00,      240, 400,  46, 26,  WHITE, 0,
     0,      plus_width, plus_height, plus_bits, 0,   "x ", 0,  "y", 0,     0},
...
int InitDisplay(int argc, char **argv) {
   ...
   */
  XrmInitialize();
  XrmParseCommand(&cmd, options, sizeof(options) / sizeof(*options), progname,
                  &argc, argv);
  ...
  *res_class = islower(*res_class) ? _toupper(*res_class) : *res_class;
  ...
  if (res) {
    ...
    *res_class = islower(*res_class) ? _toupper(*res_class) : *res_class;
    ...
    XrmParseCommand(&cmd, options, sizeof(options) / sizeof(*options), res_name,
                    &argc, argv);
  }
...
void CreateButton(int i, int off_x, int off_y, XFontStruct *f_small,
...

  {
     ...
     */
    buttons[i].xwin = XCreateSimpleWindow(dpy, mainW, off_x + buttons[i].x,
                                          off_y + buttons[i].y, buttons[i].w,
                                          buttons[i].h, 0, COLOR(BLACK), pixel);
    ...
    XChangeWindowAttributes(dpy, buttons[i].xwin, CWEventMask | CWBackingStore,
                            &xswa);
     ...
     */
    buttons[i].map =
        XCreatePixmap(dpy, buttons[i].xwin, buttons[i].w, buttons[i].h, depth);
    ...
    if (buttons[i].label != (char *)0) {
      ...
      XTextExtents(finfo, buttons[i].label, (int)strlen(buttons[i].label), &dir,
                   &fa, &fd, &xchar);
      x = (buttons[i].w - xchar.width) / 2;
      y = (1 + buttons[i].h - (xchar.ascent + xchar.descent)) / 2 +
          xchar.ascent + 1;
      XDrawImageString(dpy, buttons[i].map, gc, x, y, buttons[i].label,
    ...
    if (buttons[i].label != (char *)0) {
      ...
      y = (1 + buttons[i].h - (xchar.ascent + xchar.descent)) / 2 +
          xchar.ascent;
      XDrawImageString(dpy, buttons[i].down, gc, x, y, buttons[i].label,
                       (int)strlen(buttons[i].label));
...
void CreateKeypad(unsigned int w, unsigned int h, unsigned int offset_y,
  ...
  for (i = BUTTON_A; i <= LAST_BUTTON; i++) {
    ...
    if (buttons[i].left != (char *)0) {
      ...
      if (buttons[i].is_menu) {
        ...
        DrawSmallString(dpy, keypad->pixmap, gc, x, y, buttons[i].left,
                        strlen(buttons[i].left));
      }
    }
  }
  ...
  if (!opt_gx) {
    ...
    XCopyPlane(dpy, pix, keypad->pixmap, gc, 0, 0, last_width, last_height, x,
               y, 1);
...
void CreateBezel(unsigned int width, unsigned int height, unsigned int offset_y,
                 unsigned int offset_x, keypad_t *keypad) {
  int i, x, y;
  Pixmap pix;
  ...
  for (i = 0; i < DISP_FRAME; i++) {
    XDrawLine(dpy, keypad->pixmap, gc, (int)(DISPLAY_OFFSET_X - i - 1),
              ...
              (int)(DISPLAY_OFFSET_Y - i - 1));
    XDrawLine(dpy, keypad->pixmap, gc, (int)(DISPLAY_OFFSET_X - i - 1),
              (int)(DISPLAY_OFFSET_Y - i - 1), (int)(DISPLAY_OFFSET_X - i - 1),
              (int)(DISPLAY_OFFSET_Y + display_height + 2 * i - 1));
  }
  ...
  XDrawLine(dpy, keypad->pixmap, gc, (int)(DISPLAY_OFFSET_X + 1),
            ...
            (int)(DISPLAY_OFFSET_X + display_width - 2),
            (int)(DISPLAY_OFFSET_Y + display_height));
  XDrawLine(dpy, keypad->pixmap, gc, (int)(DISPLAY_OFFSET_X + display_width),
            (int)(DISPLAY_OFFSET_Y + 1),
...
void DrawMore(unsigned int w, unsigned int h, unsigned int offset_y,
  ...
  /* top lines */
  XDrawLine(dpy, keypad->pixmap, gc, 2, offset_y - 25, (int)(keypad->width - 4),
            offset_y - 25);
  XDrawLine(dpy, keypad->pixmap, gc, 3, offset_y - 24, (int)(keypad->width - 5),
            offset_y - 24);
  ...
  XDrawLine(dpy, keypad->pixmap, gc, 5, cut + 1, (int)(keypad->width - 7),
            cut + 1);
  ...
  pix = XCreateBitmapFromData(dpy, keypad->pixmap, (char *)hp_bits, hp_width,
                              hp_height);
  ...
  if (opt_gx) {
    ...
    XCopyPlane(dpy, pix, keypad->pixmap, gc, 0, 0, hp48gx_width, hp48gx_height,
               x, y, 1);
    XFreePixmap(dpy, pix);

    XSetFillStyle(dpy, gc, FillStippled);
    x = DISPLAY_OFFSET_X + DISPLAY_WIDTH - gx_128K_ram_width + gx_silver_x_hot +
        2;
    ...
    XCopyPlane(dpy, pix, keypad->pixmap, gc, 0, 0, hp48sx_width, hp48sx_height,
               x, y, 1);
...
void DrawKeypad(keypad_t *keypad) {
  XCopyArea(dpy, keypad->pixmap, mainW, gc, 0, 0, keypad->width, keypad->height,
            0, 0);
  return;
}
...
void CreateIcon(void) {
  ...
  if (icon_color_mode == COLOR_MODE_MONO) {
    tmp_pix =
        XCreateBitmapFromData(dpy, iconW, (char *)icon_maps[ICON_MAP].bits,
    ...
    for (p = FIRST_MAP; p <= LAST_MAP; p++) {
      tmp_pix = XCreateBitmapFromData(dpy, iconW, (char *)icon_maps[p].bits,
  ...
  XSetFillStyle(dpy, gc, FillStippled);
  icon_disp_pix =
      XCreateBitmapFromData(dpy, iconW, (char *)icon_maps[DISP_MAP].bits,
...
void refresh_icon(void) {
  ...
  XSetFillStyle(dpy, gc, FillSolid);
  if (iconW) {
    XCopyArea(dpy, icon_pix, iconW, gc, 0, 0, hp48_icon_width, hp48_icon_height,
              0, 0);
  }
}
...
int CreateWindows(int argc, char **argv) {
  ...
  if ((name = get_string_resource("title", "Title")) == (char *)0) {
    ...
    sprintf(name, "%s-%d.%d.%d", progname, saturn.version[0], saturn.version[1],
            saturn.version[2]);
  }
  ...
  mainW = XCreateWindow(dpy, RootWindow(dpy, screen), 0, 0, width, height, 0,
                        (int)depth, class, visual, mask, &xswa);
  ...
  info = XWMGeometry(dpy, screen, user_geom, def_geom, 0, &hint, &x, &y, &w, &h,
                     &hint.win_gravity);
  ...
  xswa.colormap = cmap;
  mask = CWColormap;
  iconW = XCreateWindow(dpy, RootWindow(dpy, screen), 0, 0, hp48_icon_width,
                        hp48_icon_height, 0, (int)depth, class, visual, mask,
                        &xswa);
  ...
  info = XWMGeometry(dpy, screen, user_geom, (char *)0, 0, &ih, &x, &y, &w, &h,
                     &ih.win_gravity);
...
void refresh_display(void) {
  if (shm_flag) {
    if (disp.display_update & UPDATE_DISP) {
      XShmPutImage(dpy, disp.win, disp.gc, disp.disp_image, disp.offset, 0, 5,
                   20, 262, (unsigned int)(disp.lines + 2), 0);
    }
...
#ifdef HAVE_XSHM
  if (shm_flag) {
    XShmPutImage(dpy, disp.win, disp.gc, disp.disp_image, disp.offset, 0, 5, 20,
                 262, (unsigned int)(disp.lines + 2), 0);
    if (display.lines < 63) {
      XShmPutImage(dpy, disp.win, disp.gc, disp.menu_image, 0, disp.lines - 110,
                   5, 22 + disp.lines, 262, (unsigned int)(126 - disp.lines),
                   0);
    }
    disp.display_update = 0;
  } else {
...
void get_geometry_string(Window win, char *s, int allow_off_screen) {
  ...
  sprintf(s, "%ux%u%s%d%s%d", xwa.width, xwa.height, (x_s > 0) ? "+" : "-", x,
          (y_s > 0) ? "+" : "-", y);
}
